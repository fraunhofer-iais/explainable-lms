...
def run_llm_perturber_based_on_template_path(template_path: str, text: str, tokenizer: CustomTokenizer):
    perturber = LLMBasedPerturber(
...
def test_llm_based_antonym_perturber(base_path, text, tokenizer):
    ...
    perturbations = run_llm_perturber_based_on_template_path(base_path + filename, text, tokenizer)
...
def test_leave_one_out_perturber(leave_one_out_perturber, input_data):
    ...
    perturbations = leave_one_out_perturber.perturb(text=text,
                                                    features=features)
...
def test_reorder_perturber(reorder_perturber, input_data):
    text = input_data['text']
    features = input_data['features']
    perturbations = reorder_perturber.perturb(text=text,
                                              features=features)
    assert_perturbations(text, perturbations, features)
...
def test_random_word_perturber(random_word_perturber, input_data):
    text = input_data['text']
    features = input_data['features']
    perturbations = random_word_perturber.perturb(text=text,
                                                  features=features)
    assert_perturbations(text, perturbations, features)
