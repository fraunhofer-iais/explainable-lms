...
def get_models_from_lms(lms_endpoint: str = DEFAULT_LMS_ENDPOINT) -> Dict[str, str]:
    return get_models_based_on_condition(lms_endpoint=lms_endpoint,
                                         condition=lambda value: value["model_provider"] != "SentenceTransformers")


def get_aleph_alpha_models_from_lms(lms_endpoint: str = DEFAULT_LMS_ENDPOINT) -> Dict[str, str]:
    return get_models_based_on_condition(lms_endpoint=lms_endpoint,
                                         condition=lambda value: value["model_provider"] == "AlephAlpha")


def get_models_based_on_condition(lms_endpoint: str = DEFAULT_LMS_ENDPOINT,
                                  condition: callable = lambda value: True, ) -> Dict[str, str]:
